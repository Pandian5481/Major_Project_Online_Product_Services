...
class PaytmChecksum {
	...
	static decrypt(encrypted, key) {
		try {
			if (typeof encrypted !== 'string') {
				console.error('Encrypted data is not a string:', encrypted);
	...
	static verifySignatureByString(params, key, checksum) {        
		...
		var salt = paytm_hash.substr(paytm_hash.length - 4); // Ensure paytm_hash is not null before calling substr
